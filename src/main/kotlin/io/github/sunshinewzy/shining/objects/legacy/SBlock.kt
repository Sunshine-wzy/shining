package io.github.sunshinewzy.shining.objects.legacy
...
import io.github.sunshinewzy.shining.utils.castList
...
class SBlock(val type: Material, val damage: Short = -1, var name: String = "") : ConfigurationSerializable {
    ...
    constructor(map: Map<String, Any>) : this((map["type"] as? String)?.let { Material.valueOf(it) } ?: Material.AIR,
        map["damage"] as? Short ?: -1,
        map["name"] as? String ?: "") {
        map["item"]?.let {
            if (it is ItemStack)
                item = it
