...
sealed class SCraftRecipe(
    ...
    class Size3(output: ItemStack, input: Array<ItemStack>) : SCraftRecipe(3, output, input) {
        constructor(output: ItemStack, shape: String, ingredients: Map<Char, ItemStack>) : this(
            output,
            Array(9) { ItemStack(Material.AIR) }) {
        ...
        constructor(
            ...
            line2: String,
            line3: String,
            ingredients: Map<Char, ItemStack>
        ) : this(output, "$line1\n$line2\n$line3", ingredients)
    ...
    class Size5(output: ItemStack, input: Array<ItemStack>) : SCraftRecipe(5, output, input) {
        constructor(output: ItemStack, shape: String, ingredients: Map<Char, ItemStack>) : this(
            output,
            Array(25) { ItemStack(Material.AIR) }) {
            shapeRecipe(size, shape, ingredients)
        }
    ...
    fun shapeRecipe(size: Int, shape: String, ingredients: Map<Char, ItemStack>) {
        ...
        lines.forEachIndexed forEachY@{ y, line ->
            if (y >= size) throw IllegalRecipeException(
                shape,
                "The lines of crafting shape should be less than or equal to $size, not ${lines.size}."
            )

            line.forEachIndexed forEachX@{ x, char ->
                if (char == ' ') return@forEachX
                if (x >= size) throw IllegalRecipeException(
                    shape,
                    "The characters of crafting rows should be less than or equal to $size, not ${line.length}."
                )

                input[y * size + x] = ingredients[char] ?: throw IllegalRecipeException(
                    shape,
                    "The character '$char' should be in the ingredients."
                )
            }
        }
    }
