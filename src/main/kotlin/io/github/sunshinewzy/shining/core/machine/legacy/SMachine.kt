...
abstract class SMachine(
    ...
    var defaultRecipes: Pair<KClass<out SMachineRecipe>, KClass<out SMachineRecipe>> =
        SMachineRecipe.BlockBreak::class to SMachineRecipe.ItemAddGround::class
    val displayItem =
        structure.centerBlock.toItem().setNameAndLore("§e$name", "§7----------", "§fID: §a$id", "§7----------")
    ...
    fun editRecipe(player: Player) {
        editRecipeMenu.setMultiPageAction(1, 2, 2, 4, 7, recipes.toList()) { page, order ->
            ...
            displayItem.setNameAndLore(
                "§f$first",
                "§7>> 配方类型",
                "§b输入: ${second.input.name}",
                "§e输出: ${second.output.name}"
            )
    ...
    fun getDataOrFail(sLocation: SLocation, key: String): Any =
        getData(sLocation, key)
            ?: throw IllegalArgumentException("The SLocation '${toString()}' doesn't have SMachine($id) data of $key.")
