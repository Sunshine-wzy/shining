...
class SMenu(
                ...
                if (holder.page != 0) {
                    pages[holder.page]?.let { sPage ->
                        ...
                        sPage.buttons[rawSlot]?.let {
                            it.third(this)
                            Shining.pluginManager.callEvent(
                                SMenuClickEvent(
                                    this@SMenu,
                                    id,
                                    title,
    ...
    fun createEdge(edgeItem: ItemStack) {
        val meta =
            (if (edgeItem.hasItemMeta()) edgeItem.itemMeta else Bukkit.getItemFactory().getItemMeta(edgeItem.type))
                ?: return
        meta.setDisplayName(" ")
