...
class SMenu(
        ...
        subscribeEvent<InventoryClickEvent> { 
            ...
            if(holder is SInventoryHolder<*> && this@SMenu.holder == holder) {
                if(holder.page != 0) {
                    pages[holder.page]?.let { sPage ->
                        ...
                        sPage.buttons[rawSlot]?.let { 
                            it.third(this)
                            Shining.pluginManager.callEvent(SMenuClickEvent(this@SMenu, id, title, view.asPlayer(), rawSlot, it.first, it.second, sPage.page))
                ...
                buttons[rawSlot]?.let {
                    ...
                    Shining.pluginManager.callEvent(SMenuClickEvent(this@SMenu, id, title, view.asPlayer(), rawSlot, it.first, it.second))
                }
    ...
    fun openInventory(player: Player) {
        ...
        Shining.pluginManager.callEvent(SMenuOpenEvent(this, id, title, player))
    }
    ...
    fun openInventoryByPage(player: Player, page: Int = 1) {
        ...
        player.openInventory(inv)
        Shining.pluginManager.callEvent(SMenuOpenEvent(this, id, title, player, page))
    }
